...
/* ===== 跨库智能推荐引擎 CRE v1（规则/标签重叠，仅建议，绝不生图） ===== */
...
function conceptsOf(kind, it) {
  if (kind !== 'model' && it.__c) return it.__c;   // 记忆化(静态库,满足 <50ms)
  let tokens = [];
...
/* ---- CRE v2 动态加权 ---- */
const CRE_WEIGHTS = { overlap: 1.0, usage: 0.3, affinity: 0.5, context: 0.4 };
function creMode() { return localStorage.getItem('apt_cre_mode') || 'v2'; }
function assetUsageCount(it) { return state.assetUsage[it.id] || it.usage_count || 0; }
function normUsage(count, max) { return max > 0 ? Math.log(1 + count) / Math.log(1 + max) : 0; }  // 对数归一

function scoreCandidate(src, aff, ctx, cand, kind, maxUsage, mode) {
  ...
  const usageB = normUsage(assetUsageCount(cand), maxUsage);
...
function topMatches(kind, pool, src, aff, ctx) {
  const mode = creMode();
  const maxUsage = pool.reduce((m, it) => Math.max(m, assetUsageCount(it)), 0);
  return pool
    .map((it) => { const d = scoreCandidate(src, aff, ctx, it, kind, maxUsage, mode); return Object.assign({ it, reason: buildReason(d) }, d); })
    .filter((x) => x.score > 0)
    .sort((a, b) => (b.score - a.score) || (b.overlap - a.overlap) || String(a.it.id).localeCompare(String(b.it.id)))
    .slice(0, 5);
}

function recommend(kind, item) {
  const src = conceptsOf(kind, item);
  const modelInPlay = kind === 'model' ? item : state.activeModel;       // 模特亲和层
  const aff = modelInPlay ? conceptsOf('model', modelInPlay) : new Set();
  const ctx = state.context.concepts;                                     // 会话上下文层
  const res = {};
  if (kind !== 'model') res.models = topMatches('model', state.models, src, aff, ctx);
  if (kind !== 'bg') res.backgrounds = topMatches('bg', state.backgrounds, src, aff, ctx);
  if (kind !== 'outfit') res.outfits = topMatches('outfit', state.outfits, src, aff, ctx);
  return res;
}
...
/* ===== Prompt Copilot Agent v1（纯本地规则；只写 Prompt 草稿；不生图/无外部API/无模板注入） ===== */
const CP_FIELDS = ['Model', 'Background', 'Outfit', 'Pose', 'Lighting', 'Camera', 'Style Keywords'];
const CP_POSE = ['natural standing pose', 'elegant seated pose', 'walking candid pose', 'over-the-shoulder glance', 'leaning against a wall', 'three-quarter turn pose'];
const CP_LIGHT = ['soft cinematic lighting', 'warm golden-hour light', 'studio softbox lighting', 'dramatic rim light', 'cool blue-tone lighting', 'window natural light'];
const CP_CAMERA = ['medium half-body shot, 50mm', 'close-up portrait, 85mm', 'full-body wide shot, 35mm', 'three-quarter shot, shallow depth of field', 'low-angle editorial shot', 'eye-level editorial framing'];
const CP_STYLE_BASE = 'cinematic fashion photography, high detail skin texture';

function cpTokens(text) { return (text || '').toLowerCase().split(/[\s,+，、/|]+/).filter(Boolean); }
function cpConcepts(tokens) { const s = new Set(); tokens.forEach((t) => tokenConcepts(t).forEach((c) => s.add(c))); return s; }
function cpLibHay(kind, it) {
  return (kind === 'bg'
...
function cpBestLib(kind, pool, tokens, concepts) {
...
function cpBestModel(tokens, concepts) {
  let best = null, bestScore = 0;
...
function cpRenderDraft(f) { return CP_FIELDS.map((k) => `[${k}]: ${f[k] || ''}`).join('\n'); }
function cpParseDraft(text) {
...
async function cpAutoComplete() {
  ...
  const model = cpBestModel(tokens, concepts) || state.activeModel;
...
async function cpRandom() {
  ...
  if (outfit && state.backgrounds.length) {
    const m = topMatches('bg', state.backgrounds, conceptsOf('outfit', outfit), new Set(), state.context.concepts);
...
function crePick(kind, seed) {
  ...
  const top = topMatches(kind, pool, src, aff, ctx);
